...
void CrealityDWINClass::Main_Menu_Icons() {
  ...
  if (selection == 2) {
    ...
    if (selection == 3) {
      ...
      DWIN_Draw_Rectangle(0, GetColor(2, ui_colors.highlight_box), 145, 246, 254, 345);
...
void CrealityDWINClass::Draw_Status_Area(bool icons/*=false*/) {
  ...
  if(icons) DWIN_Draw_Rectangle(1, Color_Bg_Black, 0, STATUS_Y, DWIN_WIDTH, DWIN_HEIGHT - 1);
    ...
    if (icons) { 
      ...
      DWIN_Draw_String(false, false, DWIN_FONT_STAT, GetColor(7,eeprom_settings.status_area_text), Color_Bg_Black, 25 + 3 * STAT_CHR_W + 5, 384, F("/"));
    }
    ...
    if (thermalManager.temp_hotend[0].target != hotendtarget) {
      ...
      DWIN_Draw_IntValue(true, true, 0, DWIN_FONT_STAT, GetColor(7,eeprom_settings.status_area_text), Color_Bg_Black, 3, 25 + 4 * STAT_CHR_W + 6, 384, thermalManager.temp_hotend[0].target);
    }
    if (icons) {
      DWIN_ICON_Show(ICON, ICON_StepE, 112, 417);
      DWIN_Draw_String(false, false, DWIN_FONT_STAT, GetColor(7,eeprom_settings.status_area_text), Color_Bg_Black, 116 + 5 * STAT_CHR_W + 2, 417, F("%"));
    } 
    if (planner.flow_percentage[0] != flow) {
      flow = planner.flow_percentage[0];
      DWIN_Draw_IntValue(true, true, 0, DWIN_FONT_STAT, GetColor(7,eeprom_settings.status_area_text), Color_Bg_Black, 3, 116 + 2 * STAT_CHR_W, 417, planner.flow_percentage[0]);
    }
  #endif
    ...
    if (icons) {
      ...
      DWIN_Draw_String(false, false, DWIN_FONT_STAT, GetColor(7,eeprom_settings.status_area_text), Color_Bg_Black, 25 + 3 * STAT_CHR_W + 5, 417, F("/"));
    }
    ...
    if (thermalManager.temp_bed.target != bedtarget) {
      ...
      DWIN_Draw_IntValue(true, true, 0, DWIN_FONT_STAT, GetColor(7,eeprom_settings.status_area_text), Color_Bg_Black, 3, 25 + 4 * STAT_CHR_W + 6, 417, thermalManager.temp_bed.target);
    }
  #endif
  ...
  if (icons) {
    ...
    DWIN_Draw_String(false, false, DWIN_FONT_STAT, GetColor(7,eeprom_settings.status_area_text), Color_Bg_Black, 116 + 5 * STAT_CHR_W + 2, 384, F("%"));
  }
  if (feedrate_percentage != feedrate) {
    feedrate = feedrate_percentage;
    DWIN_Draw_IntValue(true, true, 0, DWIN_FONT_STAT, GetColor(7,eeprom_settings.status_area_text), Color_Bg_Black, 3, 116 + 2 * STAT_CHR_W, 384, feedrate_percentage);
  }
...
void CrealityDWINClass::Menu_Item_Handler(uint8_t menu, uint8_t item, bool draw/*=true*/) {
  ...
  switch (menu) {
    ...
    case Advanced:
      ...
      #define ADVANCED_COLOR_THEMES (ADVANCED_TIME_FORMAT + 1)
...
bool CrealityDWINClass::isChecked(uint8_t selection, uint8_t color) {
  switch (selection){
    ...
    case 9: // Coordinates Text
      return (eeprom_settings.coordinates_text == color)?1:0;
      break;      
    default:
      return 0;                                   
  }
